      ...
      axios
        ...
        .then(res => {
          if (res.data === null || res.data === undefined) {
            this.setState({
              error: "* Userid/Password is incorrect",
              load: false
            });
          } else {
            if (
              res.data.designation === undefined ||
              res.data.designation === null
            ) {
              this.setState({
                error: "* Userid/Password is incorrect",
                load: false
        ...
        .catch(err => {
          this.setState({ error: "caught error", load: false });
          console.log(err);
        });
    } else {
      this.setState({
            ...
            <form className="login-form form-group" onSubmit={this.verifyUser}>
              ...
              <span className="login-form-logo mt-3 mb-3">
                <img
                  ...
                  src={`${window.location.origin}/assets/clg.jpg`}
                  alt="logo"
                />
              </span>
